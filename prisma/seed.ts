...
async function main() {
  ...
  // 4. Prepare bulk arrays for batch insert
  ...
  // 5. Execute bulk batch insertions
  console.log("💾 Inserting users in bulk...");
  await prisma.user.createMany({ data: usersToInsert });

  console.log("💾 Inserting instructor and students in bulk...");
  ...
  console.log(`\n⚡ Bulk seeding completed successfully in ${duration}s! (1 Instructor + ${studentNames.length} Students)`);
}
